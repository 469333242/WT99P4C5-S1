...
| WT99 FTP 文件获取 | `ftp://ftpuser:ftpuser@192.168.144.2` |
...
如果同步时间前已经生成了 `*_19800106` 默认日期会话目录，时间同步成功后会在设备空闲时自动把这些目录重命名为同步后的日期。
...
- 根目录固定为 `/sdcard`
- 支持列目录、查看文件大小、查看文件修改时间、下载文件
- 支持被动模式 `PASV/EPSV`
- 不支持上传、删除、改名、建目录或删除目录
- 同一时间只允许 1 个 FTP 客户端
- TF 卡未挂载时仍可连接 FTP，但目录和文件操作会返回不可用
- 正在录像的 `.tmp` 临时文件不会被列出，也不允许下载
