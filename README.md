...
- 默认使用 `ESP-Hosted + ESP32-C5 SDIO` 创建 Wi-Fi SoftAP 热点
- 代码保留以太网接入分支，可通过 `main/hello_world_main.c` 中的宏切换
- AP 默认地址为 `192.168.4.1`，电脑或手机连接热点后访问 `http://192.168.4.1/`；热点名称、密码、设备地址、网关和子网掩码可在网页中修改
- 当前 Wi-Fi 仅使用纯 AP；设备时间由网页通过浏览器时间同步
...
- 配置 AP 热点名称、密码、设备地址、网关、子网掩码，并查看最大客户端数
- 保存配置并提示重启生效
- 恢复网页可配置参数的默认值
- 重启设备
- 浏览 TF 卡中的照片和录像
...
- `main/User/src/wifi_connect.c`
  Wi-Fi SoftAP 创建、AP 网络地址配置与网页校时提示
...
- 热点名称、热点密码、AP 网络地址、波特率、视频源和 MIPI 分辨率保存后重启生效
