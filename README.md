...
- 默认使用 `ESP-Hosted + ESP32-C5 SDIO` 提供 Wi-Fi AP，AP 与板载以太网通过原始以太网帧定向转发；ESP-Hosted 远端 Wi-Fi 不支持 IDF 原生二层桥
- Z-1mini 吊舱固定地址为 `192.168.144.108`，电脑连接 WT99 AP 后可直接访问 `rtsp://192.168.144.108`
...
- 从 AP 切换到 STA 后会进入 STA 待确认状态，重启后需要在 STA 新地址网页点击确认；若约 `180` 秒内未确认，设备会自动保存回 AP 模式并重启，避免静态 IP 冲突或配置错误导致网页失联
- 设备时间由网页通过浏览器时间同步
...
| Z-1mini RTSP | 554 | 通过网口透传访问 `rtsp://192.168.144.108` |
...
| POST | `/api/sta_confirm` | 确认 STA 模式网页可访问，清除自动回退 AP 标记 |
...
当前默认分支为 Z-1mini 网口透传，`Z1MINI_BRIDGE_ENABLE=1`：

1. 初始化 ESP-Hosted
2. 等待 C5 SDIO transport ready
3. 创建 Wi-Fi AP 与以太网原始帧转发，AP 地址为 `192.168.144.1`
4. 短暂等待吊舱侧以太网链路；未连接时继续启动本机网页、RTSP 与 UART 服务
5. 提示通过网页同步浏览器时间

如果关闭 Z-1mini 网口透传并切换为普通 Wi-Fi 分支：
...
- `main/User/src/tcp_uart_server.c`
  TCP-UART 透传
- `main/User/src/z1mini_bridge.c`
  Z-1mini 吊舱 Wi-Fi AP 与以太网原始帧转发
...
- 切换到 STA 后必须在新地址网页确认可访问，否则设备会在约 `180` 秒后自动回退 AP 并重启
...
- Z-1mini 网口透传占用板载以太网口；启用透传时普通以太网接入分支不会同时启动
- 网页显示了多种分辨率选项，但最终是否可保存，仍由当前固件编译配置决定
- AP 与 STA 不会同时启用；切换到 STA 后如果新地址网页未在约 `180` 秒内确认，设备会自动回退到 AP 并重启
