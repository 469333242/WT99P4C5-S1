...
`device_web_config` 的职责是维护“网页可修改且需要持久化”的参数集合，例如视频源、波特率和 MIPI 视频分辨率。
...
2. 当前视频源线程在下一帧图像到达时复制所需图像数据
3. 后台照片任务完成 `视频输入帧 -> RGB565 -> JPEG -> TF 卡写入`
...
- 默认使用 `ESP-Hosted + ESP32-C5 SDIO` 提供 Wi-Fi 网络，默认场景为 AP 热点模式
- 代码保留以太网接入分支，可通过 `main/hello_world_main.c` 中的宏切换
- AP 默认地址为 `192.168.144.1`，电脑或手机连接热点后访问 `http://192.168.144.1/`；热点名称、密码、设备地址、网关和子网掩码可在网页中修改
- Wi-Fi 支持 AP 热点模式和 STA 入网模式，二者不会同时启用；STA 默认连接 `CEEWA`，密码 `52285509`，也可在网页中配置路由器 SSID、密码以及 DHCP/固定 IP
- 设备时间由网页通过浏览器时间同步

### 2.2 实时视频

- 支持 OV5647 MIPI-CSI 采集
- 支持 USB UVC 热像仪采集，当前适配 `512 x 390` Y16 原始热数据
- MIPI 摄像头和 USB 热像仪通过网页配置切换，保存后重启生效，二者不会同时工作
- USB 热像仪支持网页即时设置成像效果，当前包含 `白热`、`黑热` 和 `铁红`
- 支持硬件 ISP 输出 `YUV420`
- 支持 H.264 硬件编码
- 支持 RTSP over TCP 推流
- 当前 RTSP 最大客户端数为 `2`
- USB 热像仪 RTSP 编码按 `512 x 400` 对齐，底部补黑边仅用于 H.264 宏块对齐，拍照仍保存实际 `512 x 390` 图像区域；热像仪拍照和录像复用同一份本地成像效果
...
- 录像复用当前视频源的 H.264 编码帧，由后台通过 `esp_muxer` 封装为 MP4
- MP4 默认按 `120` 秒切段
...
- 可读取 TF 卡总容量、剩余容量、预估剩余录像时间、预估可拍照数量
...
- 查看设备当前时间、RTSP 地址、TCP-UART 入口、TF 卡状态与读写测速、RTSP/AP 客户端数
- 查看 TF 卡总容量、剩余容量、预估拍照数量和预估录像时长
- 配置 UART0/UART1 波特率
- 配置视频源，可在 `USB 热像仪` 和 `MIPI 摄像头` 间切换
- 配置 MIPI 视频分辨率档位；选择 USB 热像仪时该位置切换为热像仪成像效果设置
- 即时设置 USB 热像仪成像效果，支持 `白热`、`黑热`、`铁红`
- 配置 AP/STA 网络场景；AP 模式可设置热点名称、密码、设备地址、网关和子网掩码，STA 模式可设置路由器 SSID、密码和 DHCP/固定 IP
- 保存配置并提示重启生效
- 恢复网页可配置参数的默认值
- 重启设备
- 浏览 TF 卡中的照片和录像
...
- 视频源选择
- MIPI 视频分辨率档位
- Wi-Fi 网络场景选择（AP 热点模式 / STA 入网模式）
- AP 热点名称与密码
- AP 设备地址、网关和子网掩码
- STA 路由器 SSID、密码、DHCP/固定 IP、网关和子网掩码，默认 SSID 为 `CEEWA`，默认密码为 `52285509`
...
- USB 热像仪使用固定采集规格，MIPI 分辨率档位只对 MIPI 摄像头生效
- 配置保存后需要重启设备生效
- 恢复默认配置只重置网页可配置参数
- 恢复默认配置不会删除 TF 卡中的照片和录像
...
| POST | `/api/thermal_effect` | 即时设置 USB 热像仪本地成像效果 |
...
- `/api/thermal_effect` 使用 `application/x-www-form-urlencoded` 提交 `effect=0/1/2`，分别对应 `白热`、`黑热`、`铁红`；该设置为即时运行参数，不写入 NVS
...
3. 按网页配置初始化 Wi-Fi AP 或 STA，并等待网络就绪
4. 提示通过网页同步浏览器时间
...
4. 启动 RTSP 服务
5. 启动 `UART0` TCP 透传服务
6. 按网页配置初始化 MIPI 摄像头或 USB 热像仪模块

说明：

- 选择 MIPI 摄像头时调用 `camera_init()` 完成 MIPI 相关初始化
- 选择 USB 热像仪时启动 UVC 采集与热像仪 RTSP 本地成像编码链路
- 两种视频源不会同时工作；切换视频源保存后需要重启
...
- Wi-Fi 网络场景、AP 热点参数、STA 入网参数、波特率、视频源和 MIPI 分辨率保存后重启生效
- USB 热像仪成像效果通过 `/api/thermal_effect` 即时生效，不需要重启，也不会写入 NVS
- 网页恢复默认配置同样需要重启后完全生效
- 时间同步成功后，后续新保存文件会使用真实时间命名

### 9.4 USB 热像仪成像效果

当前 USB 热像仪输出的是 `512 x 390` 的 `Y16` 原始热数据，网页成像效果不直接依赖热像仪内部调色，而是在本机编码前完成本地转换：

1. 热像仪采集任务持续保存最新一帧 `Y16` 数据，并记录该帧最小值和最大值。
2. RTSP 任务取最新帧时，先按当前帧的最小值和最大值把每个 16bit 热值线性拉伸到 `0 ~ 255` 灰度。
3. 根据当前 `usb_thermal_camera_effect_t` 选择成像效果：
   - `白热`：灰度值越大，亮度越高。
   - `黑热`：对白热灰度做反相，灰度值越大，亮度越低。
   - `铁红`：先把灰度映射为黑红、红橙、黄白过渡的 RGB 伪彩，再按 BT.601 近似公式转换为 YUV。
4. 转换后的数据写入 H.264 硬件编码器使用的 `ESP_H264_RAW_FMT_O_UYY_E_VYY` 输入格式；每两个像素共用一个色度值，偶数行写 U，奇数行写 V。
5. RTSP 推流、自动录像和网页拍照都复用这份转换后的本地图像，因此切换热像仪成像效果后，实时视频、录像和照片保持一致。

厂商私有 USB 成像命令目前通过 `USB_THERMAL_CAMERA_VENDOR_EFFECT_CMD_ENABLE` 默认关闭。原因是当前实际视频链路使用 Y16 原始热数据，本地伪彩已经能控制 RTSP/录像/拍照颜色；同步下发厂商控制命令可能阻塞网页请求。
...
- AP 与 STA 不会同时启用；切换到 STA 后如果路由器 SSID、密码或静态 IP 配置错误，设备会持续等待 STA 连接，不会自动回退到 AP
- USB 热像仪成像效果当前为本地 Y16 伪彩转换，厂商私有 USB 成像命令默认关闭
