/**
 * @file wifi_connect.h
 * @brief Wi-Fi SoftAP 模块头文件
 *
 * 当前设备工作在纯 AP 模式，电脑或手机连接设备热点后访问网页、RTSP 和 TCP-UART 服务。
 */
...
typedef struct {
    ...
    bool        use_static_ip;  // 兼容旧网页配置结构，纯 AP 模式固定为 true
 ...
 * 纯 AP 模式下 AP 网络地址默认使用 WIFI_AP_* 参数。
 */
...
/* ================================================================== */
/**
 * @brief 初始化 Wi-Fi SoftAP
 *
 * 设备会创建固定热点并启动 DHCP server。调用前需确保 esp_netif 和事件循环已初始化，
 * 且 ESP-Hosted SDIO transport 已就绪。
 *
 * @return ESP_OK 成功
 */
 ...
 * @brief 等待系统时间有效
 *
 * 纯 AP 模式下由网页通过浏览器时间调用 /api/time 完成校时。
