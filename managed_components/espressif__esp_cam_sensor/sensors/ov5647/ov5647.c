...
#define OV5647_AE_TARGET_DEFAULT (0x34)
...
static esp_err_t ov5647_set_AE_metering_weights(esp_cam_sensor_device_t *dev)
{
    esp_err_t ret = ESP_OK;

    /*
     * OV5647 AEC uses a 4x4 weighted average window.
     * Keep the center cells heavier and the outer ring lighter so large dark
     * or bright foreground objects have less impact on global exposure.
     *
     * 1 1 1 1
     * 1 3 3 1
     * 1 3 3 1
     * 1 1 1 1
     */
    ret |= ov5647_set_reg_bits(dev->sccb_handle, 0x3a20, 1, 1, 0x00);
    ret |= ov5647_write(dev->sccb_handle, 0x5688, 0x11);
    ret |= ov5647_write(dev->sccb_handle, 0x5689, 0x11);
    ret |= ov5647_write(dev->sccb_handle, 0x568a, 0x31);
    ret |= ov5647_write(dev->sccb_handle, 0x568b, 0x13);
    ret |= ov5647_write(dev->sccb_handle, 0x568c, 0x31);
    ret |= ov5647_write(dev->sccb_handle, 0x568d, 0x13);
    ret |= ov5647_write(dev->sccb_handle, 0x568e, 0x11);
    ret |= ov5647_write(dev->sccb_handle, 0x568f, 0x11);
...
static esp_err_t ov5647_set_format(esp_cam_sensor_device_t *dev, const esp_cam_sensor_format_t *format)
{
    ...
    ret = ov5647_set_AE_metering_weights(dev);
    ESP_RETURN_ON_FALSE(ret == ESP_OK, ret, TAG, "set ae metering weight failed");
